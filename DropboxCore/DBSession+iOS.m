...
- (void)linkUserId:(NSString *)userId fromController:(UIViewController *)rootController {
    ...
    NSURL *dbURL =
    [NSURL URLWithString:[NSString stringWithFormat:@"%@://%@/connect", kDBProtocolDropbox, kDBCDropboxAPIVersion]];
    if ([[UIApplication sharedApplication] canOpenURL:dbURL]) {
        ...
        urlStr = [NSString stringWithFormat:@"%@://%@/%@/connect_login?k=%@&s=%@&state=%@&easl=1%@",
                  kDBCProtocolHTTPS, kDBCDropboxWebHost, kDBCDropboxAPIVersion,
                  consumerKey, secret, nonce, userIdStr];
        UIViewController *connectController = [[[DBCConnectController alloc] initWithUrl:[NSURL URLWithString:urlStr] fromController:rootController session:self] autorelease];
...
- (BOOL)handleOpenURL:(NSURL *)url {
    NSString *expected = [NSString stringWithFormat:@"%@://%@/", [self appScheme], kDBCDropboxAPIVersion];
    ...
    if ([methodName isEqual:@"connect"]) {
        NSDictionary *params = [DBCSession parseURLParams:[url query]];
