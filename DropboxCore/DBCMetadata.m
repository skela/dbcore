...
- (id)initWithDictionary:(NSDictionary*)dict {
    if ((self = [super init])) {
        ...
        if ([dict objectForKey:@"modified"]) {
            lastModifiedDate = 
                [[[DBCMetadata dateFormatter] dateFromString:[dict objectForKey:@"modified"]] retain];
        }

        if ([dict objectForKey:@"client_mtime"]) {
            clientMtime =
                [[[DBCMetadata dateFormatter] dateFromString:[dict objectForKey:@"client_mtime"]] retain];
        }
        ...
        if ([dict objectForKey:@"contents"]) {
            ...
            for (NSDictionary* subfileDict in subfileDicts) {
                DBCMetadata* subfile = [[DBCMetadata alloc] initWithDictionary:subfileDict];
...
- (BOOL)isEqual:(id)object {
    ...
    if (![object isKindOfClass:[DBCMetadata class]]) return NO;
    DBCMetadata *other = (DBCMetadata *)object;
