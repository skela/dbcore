...
{
    return [self initWithURLRequest:aRequest andInformTarget:aTarget selector:aSelector startImmediately:YES];
}

- (id)initWithURLRequest:(NSURLRequest *)aRequest andInformTarget:(id)aTarget selector:(SEL)aSelector startImmediately:(BOOL)startImmediately
{
    if ((self = [super init]))
    {
...
- (id)parseResponseAsType:(Class)cls
{
    ...
    if (![res isKindOfClass:cls]) {
        [self setError:[NSError errorWithDomain:DBCErrorDomain code:DBCErrorInvalidResponse userInfo:userInfo]];
        return nil;
    }
    return res;
}
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    if (resultFilename && [self statusCode] == 200) {
        @try {
            ...
            [self setError:[NSError errorWithDomain:DBCErrorDomain code:DBCErrorInsufficientDiskSpace userInfo:userInfo]];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if (self.statusCode != 200) {
        ...
        [self setError:[NSError errorWithDomain:DBCErrorDomain code:self.statusCode userInfo:errorUserInfo]];
        ...
        if (!fileAttrs) {
            ...
            [fileManager removeItemAtPath:tempFilename error:nil];
            [self setError:[NSError errorWithDomain:DBCErrorDomain code:DBCErrorGenericError userInfo:self.userInfo]];
        } else {        
...
- (void)connection:(NSURLConnection *)connection willSendRequestForAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
    ...
    if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
        ...
        SecTrustSetAnchorCertificates(serverTrust, (__bridge CFArrayRef)[DBCRequest rootCertificates]);
...
+ (bool)isRevokedCertificate:(SecCertificateRef)certificate {
    ...
    const CFDataRef serial = [DBCRequest copySerialFromCertificate:CFDataGetBytePtr(cfder) certificateLength:CFDataGetLength(cfder)];
    CFRelease(cfder);
#else
